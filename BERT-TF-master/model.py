...
class BertNer(tf.keras.Model):
    ...
    def __init__(self, bert_model,float_type, num_labels, max_seq_length, final_layer_initializer=None):
        ...
        #sequence_output shape: batch_size, (max_)seq_len, hidden_len (length of embedding vector)
...
class BertRe(tf.keras.Model):
    ...
    def __init__(self, bert_model,float_type, num_labels, max_seq_length, final_layer_initializer=None):
        ...
        self.bert = tf.keras.Model(inputs=[input_word_ids, input_mask, input_type_ids],outputs=[pooled_output])
        #pooled_output shape: batch_size, hidden_size (just used the vector of the first token)
        if type(bert_model) == str:
            init_checkpoint = os.path.join(bert_model,"bert_model.ckpt")
            checkpoint = tf.train.Checkpoint(model=self.bert)
            checkpoint.restore(init_checkpoint).assert_existing_objects_matched()
    ...
    def call(self, input_word_ids,input_mask=None,input_type_ids=None,valid_mask=None, **kwargs):
        pooled_output = self.bert([input_word_ids, input_mask, input_type_ids],**kwargs)
        
        valid_output = pooled_output
