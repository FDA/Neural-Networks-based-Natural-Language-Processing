...
parser.add_argument('--tokenized_answer_path', type=str, default=None,  help='Path to tokenized golden dataset. ex) token_label_test.tsv')
...
def detokenize(pred_token_test_path, pred_label_test_path, tokenized_gold_path):
    """
    ...
    # read predicted
    pred = {'toks':[], 'labels':[], 'reallabels': []} # dictionary for predicted tokens and labels.
    with open(pred_token_test_path,'r') as in_tok, open(pred_label_test_path,'r') as in_lab, open(tokenized_gold_path, 'r') as in_real: #'token_test.txt'
        for lineIdx, (lineTok, lineLab, lineReal) in enumerate(zip(in_tok, in_lab, in_real)):
            lineTok = lineTok.strip()
            pred['toks'].append(lineTok)
            
            lineLab = lineLab.strip()
            lineReal = lineReal.split()[1]  #2nd column is label
            if lineIdx <= -100:
                print("lineIdx is ", lineIdx, "token is ", lineTok, " label is ", lineLab, " reallabel is ", lineReal)
            #if lineLab in ['[CLS]','[SEP]', 'X']: # replace non-text tokens with O. These will not be evaluated.
            if lineReal in ['[CLS]','[SEP]']: #don't replace X just yet; need to use it to specify broken token    
                pred['labels'].append('O')
                pred['reallabels'].append('O')
                continue
            pred['labels'].append(lineLab)
            pred['reallabels'].append(lineReal)
    ...
    assert (len(pred['toks']) == len(pred['reallabels'])), "Error! : len(pred['toks'])(%s) != len(pred['reallabels'])(%s) : Please report us "%(len(pred['toks']), len(pred['reallabels']))
    
    bert_pred = {'toks':[], 'labels':[], 'sentence':[]}
    buf = []
    c = -1
    for t, l, r in zip(pred['toks'], pred['labels'], pred['reallabels']):
        c += 1;
        if c < -100:
            print("line no. is ", c, "line token is ", t, " label is ", l, " reallabel is ",r)
        if t in ['[CLS]','[SEP]']: # non-text tokens will not be evaluated.
            ...
            buf[-1]+=t[2:]
        elif r[:1] == 'X':  #broken by Word Piece tokenizer without adding ##. note I'm checking the "real" label instead of the predicted!
            bert_pred['toks'][-1] +=t
            buf[-1]+=t
        else:
            bert_pred['toks'].append(t)
    ...
    #for t, l in zip(bert_pred['toks'], bert_pred['labels']): 
        #print("processed token is ", t, " processed label is ", l)
...
if __name__ == "__main__":
    bert_pred = detokenize(pred_token_test_path=args.token_test_path, pred_label_test_path=args.label_test_path, tokenized_gold_path=args.tokenized_answer_path)
